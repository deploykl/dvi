...
const LISTAR = async () => {
  try {
    ...
    const responseMedidaActividad = await api.get('poi/medida-actividad/', {
      headers: {
...
const ADD = async () => {
  try {
    const accessToken = getAuthToken();
    const response = await api.post('poi/medida-actividad/', {
      ...form.value,
...
const UPDATE = async () => {
  try {
    const accessToken = getAuthToken();
    const response = await api.put(`poi/medida-actividad/${form.value.id}/`, {
      ...form.value,
      MedidaActividad: form.value.MedidaActividad,
    }, {
      headers: {
